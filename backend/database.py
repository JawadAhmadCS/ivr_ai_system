...
def ensure_schema():
    with engine.connect() as conn:
        ...
        try:
            ...
            pass
        try:
            if not _column_exists(conn, "call_logs", "call_sid"):
                conn.execute(text("ALTER TABLE call_logs ADD COLUMN call_sid VARCHAR(100) NULL"))
        except Exception:
            pass
        try:
            if not _column_exists(conn, "orders", "recording_sid"):
                conn.execute(text("ALTER TABLE orders ADD COLUMN recording_sid VARCHAR(100) NULL"))
        except Exception:
            pass
        try:
            if not _column_exists(conn, "orders", "recording_url"):
                conn.execute(text("ALTER TABLE orders ADD COLUMN recording_url VARCHAR(500) NULL"))
        except Exception:
            pass
        try:
            if not _column_exists(conn, "orders", "card_no"):
                conn.execute(text("ALTER TABLE orders ADD COLUMN card_no VARCHAR(64) NULL"))
        except Exception:
            pass
        try:
